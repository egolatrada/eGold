...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option
                .setName('razon')
                .setDescription('Razón de la aprobación (opcional)')
                .setRequired(false)
        ),
    ...
    async execute(interaction, context) {
        try {
            ...
            suggestionsSystem.resolveSuggestion(messageId, 'approved', interaction.user.id);
            ...
            await interaction.reply({
                content: `✅ Sugerencia marcada como **APROBADA**.`,
                ephemeral: true
            });

            logger.info(`✅ ${interaction.user.tag} aprobó la sugerencia ${messageId}`);

        } catch (error) {
            logger.error('Error al aprobar sugerencia', error);
            await interaction.reply({
                content: '❌ Ocurrió un error al aprobar la sugerencia.',
                ephemeral: true
            }).catch(() => {});
        }
    }
